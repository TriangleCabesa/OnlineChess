...
namespace OnlineChess.Implementations
{
    ...
    {
        public ISpace? EnPessantSpace { get; set; }
        ...
        public List<ISpace> GetMoveableSpaces(IBoard board, bool checkLegalMoves = true)
        {
            ...
            if (!board.Spaces[x, y].IsOccupied)
            {
                ...
                if (firstMove && !board.Spaces[x, secondY].IsOccupied)
                {
                    ...
                    if (x > 0 && board.Spaces[x - 1, secondY].GetPiece() is Pawn && board.Spaces[x - 1, secondY].GetPiece()!.IsWhite != IsWhite)
                        (board.Spaces[x - 1, secondY].GetPiece() as Pawn)!.EnPessantSpace = board.Spaces[x, y];
            ...
            if (EnPessantSpace is not null)
            {
                possibleMoves.Add(EnPessantSpace);
            }

            return checkLegalMoves ? possibleMoves.GetLegalMoves(board, GetKing(board), Point) : possibleMoves;
        }
